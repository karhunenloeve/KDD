...
\begin{document}
  ...
  {
    \setbeamertemplate{footline}[frame number]
    \begin{frame}{Density-dased clustering: basic concepts}
        \begin{itemize}
          \item \textbf{Two parameters:}
          \begin{itemize}
            \item \textbf{\color{airforceblue}Eps}: Maximum radius of a neighborhood.
            \begin{itemize}
              \item Defines the neighborhood of a point $p$: $N_{\text{Eps}}(p) := \{q \in D \; \vert \; d(p,q) \leq \text{Eps}\}$.
              \item Distance function is still needed.
            \end{itemize}
            \item \textbf{\color{airforceblue}MinPts}: Minimum number of points in an Eps-neighborhood of a point $p$.
          \end{itemize}
          \item \textbf{{\color{airforceblue}Core-point condition:} $|N_{\text{Eps}}(p)| \geq \text{MinPts}$.}
          \begin{itemize}
...

  {
    ...
    \begin{frame}{Directly density-reachable}
      \begin{itemize}
        ...
        w.r.t. Eps, MinPts, if $q$ belongs to $N_{\text{Eps}}(p)$.
      \end{itemize}
      \centering
      \vspace{0.5cm}
      \begin{tikzpicture}
...

  {
    ...
    \begin{frame}{Density-reachable and density-connected}
      \begin{itemize}
        ...
        \begin{itemize}
          \item A point $q$ is density-reachable from a point $p$ w.r.t. Eps, MinPts, if there is a \textbf{chain of points} $p_1, \ldots, p_n$, $p_1 = p$, $p_n = q$ such that $p_i+1$ is directly density-reachable from $p_i$.\\[0.2cm]
          \centering
        ...
        \begin{itemize}
          \item A point $p$ is density-connected to a point $q$ w.r.t. Eps, MinPts, if there is a point $o$ such that both $p$ and $q$ are density-reachable from $o$. \\[0.2cm]
          \centering
          \begin{tikzpicture}[scale=1.2]
            \node[draw, circle, fill=red, scale=0.3] at (0.3,0.1) {};
            \node[draw, circle, fill=red, scale=0.3] at (0.3,0.5) {};
      ...
      \begin{columns}
        \begin{column}{0.7\textwidth}
          ...
          \begin{itemize}
            ...
            \begin{itemize}
              \item of a point $p$.
              \item Min Eps s.t. $p$ is core point.
            \end{itemize}
  ...
  {
    \setbeamertemplate{footline}[frame number]
    \begin{frame}{The STING clustering method}
      \begin{itemize}
  ...
  {
    \setbeamertemplate{footline}[frame number]
    \begin{frame}{CLIQUE: the major steps}
      \begin{itemize}
