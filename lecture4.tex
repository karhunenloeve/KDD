...
\begin{document}
...

 {
  ...
  {
    \setbeamertemplate{footline}[frame number]
    \begin{frame}{Correlation analysis for nominal data (I)} % A has distinct values => A = A' ???
      \begin{itemize}
  ...
  {
    \setbeamertemplate{footline}[frame number]
    \begin{frame}{Correlation analysis for nominal data (II)} % eventuell \hat{x} als erwartungswert explizit erklären ???
      \begin{itemize}
...
are those whose actual count is very different from the expected count.
  ...
  {
    \setbeamertemplate{footline}[frame number]
    \begin{frame}{Correlation analysis of numerical data} % Nur Diagonaleneinträge - Indizierung in Summe bis N+M ???
    \begin{itemize}
  ...
  {
    \setbeamertemplate{footline}[frame number]
    \begin{frame}{Principal component analysis: the problem (II)} % eventuell <x> als erwartungswert erklären ???
    \begin{itemize}
    ...
    \begin{align}
      \vert\vert u_i \vert\vert = \left(\sum_{j=1}^{m} u_{i, j}^2\right)^\frac{1}{2}.
    \end{align}
      \item \textbf{Goal:} find $u_*$ such that the variance of the projections of $x$ onto $u_*$, $\langle (x^{T}u_*)^2 \rangle$, becomes maximal according to the probability distribution $p(x)$.
    \end{itemize}
    \end{frame}
  }

  {
    ...
    \begin{align}
      L(w) := \langle (\frac{x^Tw}{\vert\vert w \vert\vert})^2 \rangle = \langle (x^Tu)^2 \rangle = u^T \langle xx^T \rangle u = u^T \mathbf{C} u,
    \end{align}
    with
    \begin{align}
    ...
    \begin{align}
      \langle (x^Tu)^2 \rangle = \langle (x^Tu)(x^Tu) \rangle = u^T\langle xx^T \rangle u = u^T \mathbf{C}u,
    \end{align}
    and it holds that
    \begin{align}
...

  {
    ...
    \begin{align}
    \nabla_w L(w) &= 0,\\
    \frac{\mathbf{C}w \vert\vert w \vert\vert^2 - (w^T \mathbf{C} w)w}{\vert\vert w \vert\vert^4} &= 0.
    \end{align}
    This yields the following eigenvalue problem:
    \begin{align}
      \mathbf{C}w = \frac{w^T\mathbf{C}w}{\vert\vert w \vert\vert^2} \cdot w = L(w) \cdot w = \lambda \cdot w.
    \end{align}
...

  {
    ...
    \begin{itemize}
      ...
      \end{align}
      \item The $k$th component $x^Tu_k$ is along the direction of the eigenvector $u_k$ pointed to the $k$th biggest eigenvalue $\lambda_k$ of the covariance matrix: % Satz macht keinen sinn: skalarer Wert zeigt in richtung eigenvektor auf eigenwert???
      \begin{align}
